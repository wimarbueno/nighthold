...
e.each(function(){c(this).closest(d).find(k).filter(function(){return c(this).attr("data-action")===b}).click();e.prop("checked",!1)});this.setSelectActionState(!1)},insertAttachment:function(a,b,c){c=c||"image";var d=a.data("attachment-id");if(d&&this.editor){var e=a.find(this.options.templateThumb).attr("src");a=a.find(this.options.templateView).attr("href");var f,g={id:d,img:e};if("full"==b)b="[ATTACH=full]"+d+"[/ATTACH]","image"==c?f='<img src="{{img}}" data-attachment="full:{{id}}" alt="" />':
"video"==c?f='<span contenteditable="false" draggable="true" class="fr-video fr-dvi fr-draggable fr-deletable"><video data-xf-init="video-init" data-attachment="full:{{id}}" src="{{img}}" controls></video></span>':"audio"==c&&(f='<span contenteditable="false" draggable="true" class="fr-audio fr-dvi fr-draggable fr-deletable"><audio data-attachment="full:{{id}}" src="{{img}}" controls></audio></span>'),g.img=a;else{if(!e||"image"!==c)return;b="[ATTACH]"+d+"[/ATTACH]";f='<img src="{{img}}" data-attachment="thumb:{{id}}" alt="" />'}f=
Mustache.render(f,g);XF.insertIntoEditor(this.$target,f,b,"[data-attachment-target=false]")}},deleteAttachment:function(a,b){b=b||"image";var d=a.data("attachment-id");if(d){var f=this;XF.ajax("post",this.manageUrl,{delete:d},function(b){b.delete&&f.removeFileRow(a)},{skipDefaultSuccess:!0});var e=new RegExp("^[a-z]+:"+d+"$","i"),g=new RegExp("\\[attach[^\\]]*\\]"+d+"\\[/attach\\]","gi");XF.modifyEditorContent(this.$target,function(a){var d=a.ed;"image"==b||"file"==b?d.$el.find("img[data-attachment]").filter(function(){return e.test(c(this).attr("data-attachment"))}).each(function(){d.image.remove(c(this))}):
("video"==b||"audio"==b)&&d.$el.find(b+"[data-attachment]").filter(function(){return e.test(c(this).attr("data-attachment"))}).each(function(){c(this).parent().remove()})},function(a){var b=a.val();b=b.replace(g,"");a.val(b)},"[data-attachment-target=false]")}},cancelUpload:function(a){var b=a.data("file");a.data("attachment-id")||b&&1==b.progress()||(this.flow.removeFile(b),this.flow.isUploading()||this.setUploading(!1),this.removeFileRow(a))},uploadQueryParams:function(){return{_xfToken:XF.config.csrf,
_xfResponseType:"json",_xfWithData:1}},applyUploadTemplate:function(a){a=c(c.parseHTML(Mustache.render(this.template,a)));var b=this.options.fileRow;return a.filter(function(){return c(this).is(b)})},removeFileRow:function(a){a.remove();this.toggleInsertMultiRow();this.getFileRows().length||(this.legacyMode?this.$filesContainer.removeClass("is-active"):this.$container.removeClass("is-active"),XF.layoutChange())},removeInsertButtons:function(a){a.find(this.options.insertRow+","+this.options.insertMultiRow).remove();
XF.layoutChange()},toggleInsertMultiRow:function(){this.checkboxClick();var a=this.$filesContainer.find(this.options.actionButton).filter(":not([data-action=delete])").closest(this.options.fileRow),b=this.$container.find(this.options.insertMultiRow);1<a.length?b.addClass("is-active"):b.removeClass("is-active");XF.layoutChange()},resetAttachments:function(){var a=this;a.getFileRows().each(function(){a.removeFileRow(c(this))})},getFileRows:function(){return this.$filesContainer.find(this.options.fileRow)}});
XF.AttachmentOnInsert=XF.Element.newHandler({options:{fileRow:".js-attachmentFile",href:null,linkData:null},loading:!1,init:function(){var a=this.$target.closest(this.options.fileRow);a.length&&this.options.href||console.error("Cannot find inserted row or action to perform.");a.on("attachment:row-inserted",XF.proxy(this,"onAttachmentInsert"))},onAttachmentInsert:function(a,b,c){if(!this.loading){var d=this;XF.ajax("post",this.options.href,this.options.linkData||{},XF.proxy(this,"onLoad")).always(function(){d.loading=
