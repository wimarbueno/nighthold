<?php
    ...
    class Achievements {
        ...
        private static function CalculateAchievementPoints() {
            ...
            self::$achievements_points = DB::table('achievement')->whereIn('id', self::GetAchievementsIDs())->sum('points');
            return true;
        }
        ...
        private static function SortAchievements() {
            ...
            $categories = DB::table('achievement')->select(['ID', 'Category'])->whereIn('ID', self::GetAchievementsIDs())->get();
            if(!$categories) {
                return false;
            }
            $storage = array();
            foreach($categories as $category) {
                if(!isset($storage[$category->Category])) {
                    $storage[$category->Category] = array();
                }
                $current_id = $category->ID;
                foreach(self::$achievements_storage as $achievement) {
                    if($achievement->achievement == $current_id) {
                        $storage[$category->Category][] = $achievement;
                        break;
                    }
                }
            }
        ...
        public static function GetAchievementsTotalComplete($id): int
        {
            return DB::table('achievement')->where('Category', $id)->count('id');
        }

        public static function GetAchievementsTotal($id): int
        {
            return DB::table('achievement')->where('Category', $id)->count('id');
        }
        ...
        private static function FindAchievementInDB($achievement_id) {
            return DB::table('achievement')->where('id', $achievement_id)->first();
        }
        ...
        public static function BuildCategoriesTree($statistics = false) {
            if($statistics) {
                $categories = DB::table('achievement_category')->where('parentCategory', 1)->get();
            }
            else {
                $categories = DB::table('achievement_category')->selectRaw("parentCategory = -1 AND id <> 1")->get();
            }
            if(!$categories) {
                return false;
            }
            ...
            $i = 0;
            foreach($categories as $category) {
                $child_categories = DB::table('achievement_category')->where('parentCategory', $category['id'])->first();
        ...
        private function IsAchievementHasChildAchievementCompleted($achievement_id, $category) {
            $child_achievement = DB::table('achievement')->where('parentAchievement', $achievement_id)->first();
            $skip = array();
        ...
        public static function GenerateAchievementChain($original_achievement_id, $category_id) {
            ...
            $child_id = DB::table('achievement')->select('parentAchievement', 'points')->where('id', $achievement_id)->first();
            ...
            $tmp_chain = array();
            $achievement_id = $original_achievement_id;
            $child_id = DB::table('achievement')->select('id', 'points')->where('parentAchievement', $achievement_id)->first();
            while($child_id['id'] > 0) {
                $tmp_chain[] = array(
                    'current'   => $achievement_id,
                    'next'      => $child_id['id'],
            ...
            $chain[$index] = array(
                ...
                'next'      => 0,
                'completed' => (int) self::IsAchievementCompleted($achievement_id, $category_id),
                'points'    => DB::table('achievement')->select('points')->where('id', $achievement_id)->first()
            );
            return $chain;
        }
        ...
        public static function AchievementCategoryHome($category) {
            return DB::table('achievement_category')->where("Parent", $category)->whereNotBetween('ID', [15275, 15237, 1])->get();
        }

        public static function AchievementCategory($category) {
            $achievement_category = array();
            $achievements = DB::table('achievement_category')->where("Parent", $category)->whereNotBetween('ID', [15275, 15237, 1])->get();
            dd($achievements);
            if(!$achievements) {
                return false;
            }
        ...
        public static function GetCategoryInfoFromDB($category) {
            return DB::table('achievement_category')->select('ID', 'Name_Lang')->where('id', $category)->first();
        }
