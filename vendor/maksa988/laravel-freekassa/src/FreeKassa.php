...
class FreeKassa
{
    ...
    public function getPayUrl($amount, $order_id, $phone = null, $email = null, $user_parameters = [])
    {
        ...
        $query['m'] = setting('freekassa.project_id');
        ...
        $query['lang'] = setting('freekassa.locale', 'ru');

        // Payment currency
        if (! is_null(setting('freekassa.currency'))) {
            $query['i'] = setting('freekassa.currency');
        }

        $query['s'] = $this->getFormSignature(
            setting('freekassa.project_id'),
            $amount,
            setting('freekassa.secret_key'), $order_id
        );
